...
import com.meetutech.baosteel.widget.video.FocusSurfaceView;
import com.meetutech.baosteel.widget.video.MyVideoView;
import com.meetutech.baosteel.widget.video.RecordedButton;
...
public class VideoCameraActivity extends BaseActivity implements View.OnClickListener{
  ...
  private static VideoCameraActivity instance;
    ...
    new AsyncTask<Void, Void, String>() {
      ...
      protected String doInBackground(Void... params) {
        ...
        for (int x = 0; x < mMediaObject.getMediaParts().size(); x++) {
          ...
          String mp4Path = BSApplication.VIDEO_PATH+"/"+x+".mp4";
          ...
          if(cameraTypeList.get(x) == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            rotateMp4 = BSApplication.VIDEO_PATH+"/"+x+"_rotate.mp4";
        ...
        for (int x = 0; x < pathList.size(); x++) {
          ...
          String ts = BSApplication.VIDEO_PATH+"/"+x+".ts";
          mp4ToTs(path, ts);
          tsList.add(ts);
        }

        String output = BSApplication.VIDEO_PATH+"/finish.mp4";
        boolean flag = tsToMp4(tsList, output);
        if(!flag) output = "";
        deleteDirRoom(new File(BSApplication.VIDEO_PATH), output);
        return output;
      }
      @Override
  ...
  public void onClick(View v) {
    switch (v.getId()){
      ...
      case R.id.iv_next:
        ...
        Intent intent = new Intent(VideoCameraActivity.this, EditVideoActivity.class);
        intent.putExtra("path", BSApplication.VIDEO_PATH+"/finish.mp4");
  ...
  public static VideoCameraActivity getInstance() {
    return instance;
  }
}
